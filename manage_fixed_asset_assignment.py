...
class ReturnAssignmentResource(Resource):
   #@jwt_required()
    def patch(self, assignment_id):
        ...
        update_assignment_query = text("UPDATE assignment SET return_date = :return_date WHERE id = :assignment_id;")
        update_asset_status_query = text("UPDATE asset SET status = 'available' WHERE id = :asset_id;")
        ...
        try:
            with db.engine.begin() as connection:
                ...
                return {'message': 'Asset returned successfully', 'return_date': return_date}, 200
        except Exception as e:
            return {'error': str(e)}, 500
